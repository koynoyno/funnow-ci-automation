...
雖然[不建議](https://martinfowler.com/bliki/PageObject.html)在頁面物件中使用斷言，但系統測試時行為有時可能不可預測，因此這個概念驗證使用了頁面物件內的定位器，以簡化流程。

#### 為何預訂流程測試不穩定？

存在一個問題，即如果使用者在相同時間進行太多預訂，他們將突然被重定向到選擇時間的頁面，而不是支付頁面，在這種情況下，網站沒有處理來自 API 的錯誤。
